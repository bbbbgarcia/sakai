...
import org.sakaiproject.gradebookng.tool.panels.GradeRubricPanel;

public class ViewGradeRubricAction extends InjectableAction implements Serializable {

	private static final long serialVersionUID = 1L;

	public ViewGradeRubricAction() {
	}
	...
	public ActionResponse handleEvent(final JsonNode params, final AjaxRequestTarget target) {
		...
		final GbModalWindow window = gradebookPage.getGradeRubricWindow();

		window.setAssignmentToReturnFocusTo(assignmentId);
		window.setStudentToReturnFocusTo(studentUuid);
		window.setContent(new GradeRubricPanel(window.getContentId(), Model.ofMap(model), window));
		window.show(target);
