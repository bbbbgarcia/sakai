 ...
 */
public class GradeRubricPanel extends BasePanel {

	private static final long serialVersionUID = 1L;

	private final ModalWindow window;

	public GradeRubricPanel(final String id, final IModel<Map<String, Object>> model, final ModalWindow window) {
		super(id, model);
		this.window = window;
	}
	...
	public void onInitialize() {
		...
		final ListView<GbGradeLog> listView = new ListView<GbGradeLog>("log", gradeLog) {
			...
			protected void populateItem(final ListItem<GbGradeLog> item) {
				...
				final GbUser grader = GradeRubricPanel.this.businessService.getUser(gradeLog.getGraderUuid());
		...
		add(new GbAjaxLink("done") {
			...
			public void onClick(final AjaxRequestTarget target) {
				GradeRubricPanel.this.window.close(target);
			}
		});
		...
		GradeRubricPanel.this.window.setTitle(
